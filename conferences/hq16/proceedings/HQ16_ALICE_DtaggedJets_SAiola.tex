...
Furthermore, 
%the details of the charm quark fragmentation can be the subject of a more careful study in which the kinematic observables 
%of both the jet and the charmed hadron are available at the same time. In particular 
...
%The TOF detector is a large area 
...
%The simulated statistics (about 300 M events) is close to the total statistics accumulated by ALICE in MB \pp\ collisions in 2010. 
...
%For this study only the tracks of charged particles are used for jet reconstruction (\emph{charged jets}). 
%Since they miss the momentum
...
%The D-tagged jets are identified at detector-level, 
...
%These matched pairs are used to build a response matrix that relates detector-level observables with their corresponding generator-level observables. 
