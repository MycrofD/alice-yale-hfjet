...
using rare hard probes. The ALICE Collaboration has presented new results on the jet shape observables in \pPb\ and \PbPb\ collision. 
Using data from Run-2, the elliptic flow of open and hidden charm mesons have been measured with much better precision,
...
\label{fig:zgppb}
\end{figure}
%ALICE plans to perform this measurement in \PbPb\ collisions using data collected during Run-2.
%CMS uses a cut $\Delta R > 0.1$, which is expected to potentially change the physics interpretation. 
%Thanks to their better angular resolution, charged-only jets reconstructed using ALICE tracking system (ITS+TPC) could allow to relax this constraint.
