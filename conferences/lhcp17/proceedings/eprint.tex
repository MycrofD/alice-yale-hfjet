...
using rare hard probes. The ALICE Collaboration has presented new results on jet substructure in \pPb\ and \PbPb\ collisions which show no modification
...
correlated with high event activity has been confirmed measuring the self-normalized semi-inclusive hadron+jet cross section.
\end{Abstract}
\vfill
...
\label{eq:zg}
\end{equation}
In order to find the two hard subjets a ``soft drop'' algorithm is employed. Jets are first identified via the \antikt\ algorithm, then reclustered using Cambridge/Aachen algorithm.
